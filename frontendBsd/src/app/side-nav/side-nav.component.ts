...
export class SideNavComponent implements OnInit {
  ...
  show: boolean = true;
  href: string = "";
  searching: Topics = new Topics();
  // navigateStr: string = "";

  constructor(private shareData: ShareDataService, private dialog: MatDialog,
    private topicsService: TopicsService) {
    this.searching.icon = "search";
    this.searching.title = "חיפוש ראשי";
    try {
  ...
  openMainSearching(): void {
    try {
      const dialogRef = this.dialog.open(MainSearchComponent, {
        width: '350px', data: { }
      });
